...
export class ViewFilesComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ['name', 'type', 'size', 'lastModifiedDate','download'];
  ...
  handleDownloadFile(filename: string, type: string): void {
    this.uploadFileService.getFileByName(filename).subscribe(data => this.downloadFile(data, type));
  }
